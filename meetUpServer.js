...
var GroupHang = function(date,time,topic) {
	this.date = date,
	this.topic = topic,
	this.headCount = headCount
}

var Rsvp = function(names, email) {
	this.name = name,
	this.email = email
}
  ...
  var hang = function() {
	fs.readFile('groupHang.json', data, function (err) {
  	if (err) {
  		...
  		welcome = ("Welcome to devMeetUp. Next groupHang is scheduled for " + date +". We will be examining " + topic + ". Type 'rsvp' to sign up.");
  	...
  	if(rsvpInfo ===2){
  		if(data === ""){
  			...
  			fs.readFile('rsvpYes.json'), function(err, data){
  				...
  				fs.writeFile('rsvpList.json', JSON.stringify(rsvpList), function(err){
  					if(err){
  						console.log(err);
  					} else{
